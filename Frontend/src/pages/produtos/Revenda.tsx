...
export default function Revenda() {
  ...
  const { data: historyData, isLoading: isLoadingHistory } = useQuery({
    queryKey: ["product_history", currentRecord?.id, historyPage, historyPerPage],
    queryFn: () =>
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    schema?.fields.forEach((field) => {
      ...
      // Do not submit hidden fields based on tipo
    ...
    // Number casting
    if (dataObj.preco_venda !== undefined && dataObj.preco_venda !== '') {
      dataObj.preco_venda = Number(dataObj.preco_venda);
    }
    ...
    if (selectedTipo === "Consumivel" && !dataObj["preco_venda"]) {
      dataObj["preco_venda"] = 0;
    }
  ...
  let data = paginatedResponse?.items || (Array.isArray(paginatedResponse) ? paginatedResponse : []);
  let totalItems = paginatedResponse?.total || data.length;
  let calculatedPages = Math.max(1, Math.ceil(totalItems / perPage));
  
  // se o backend retornou tudo na mesma chamada (não paginou)
  if (data.length > perPage) {
    totalItems = data.length;
    calculatedPages = Math.max(1, Math.ceil(totalItems / perPage));
  ...
  const pagination = {
    page: paginatedResponse?.page || page,
    total: totalItems,
    pages: paginatedResponse?.pages || calculatedPages
  };
  ...
  const tableColumns = React.useMemo<ColumnDef<any>[]>(() => {
    ...
    const hiddenColumns = {
      Consumivel: [],
      Acabado: ["stock_atual", "preco_compra"],
      Revenda: [],
    };
    const schemaFiltrado = schema.columns.filter(
     ...
     <DataTable
          ...
          <>
            <div className="flex justify-between w-full">
              <div className="flex gap-2">
                {currentRecord && currentRecord.tipo === "Acabado" && (
                  <button
                    type="button"
                    onClick={() =>
                      navigate(`/produtos/${currentRecord.id}/receita`)
                    }
                    className="px-5 py-2 rounded-lg font-medium border border-primary text-primary hover:bg-primary/10 transition-colors"
                  >
                    Gerir Receita
                  </button>
                )}
          ...
          >
            {currentRecord && currentRecord.tipo === "Acabado" && (
              <div className="bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 p-3 rounded-lg text-sm border border-blue-100 dark:border-blue-800">
                Este produto ainda não possui uma receita técnica. Adicione
                ingredientes para compor o custo!
              </div>
            )}
            ...
            <div className="bg-white dark:bg-surface-dark border border-gray-200 dark:border-gray-800 rounded-xl overflow-hidden mb-6">
              <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      step="0.01"
                      name="preco_venda"
                      defaultValue={currentRecord?.preco_venda || ""}
                      required
                      className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary/50"
                    />
                      <span className="absolute right-10 top-1/2 -translate-y-1/2 text-gray-500 text-sm">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    name="taxa_iva_id"
                    defaultValue={currentRecord?.taxa_iva_id || ""}
                    className="w-full px-3 py-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary/50"
                  >
                    <option value="">Selecione...</option>
                    {schema.fields
                      .find((f) => f.name === "taxa_iva_id")
                      ?.options?.map((opt: any) => (
                        <option key={opt.value} value={opt.value}>
                          {opt.label}
                        </option>
      ...
      >
        <DataTable
          ...
          paginationState={{ pageIndex: historyPage - 1, pageSize: historyPerPage }}
          onPaginationChange={(updater) => {
            const current = {
