...
export default function CaixaPOS() {
  ...
  const displayProducts = posProducts.filter((p: any) => {
    ...
    const matchCategory = activeCategory === 'Tudo' || pCategory === activeCategory;
    
    // Hide ingredients/consumables from POS
    if (activeCategory === 'Tudo' && pCategory === 'Ingredientes') return false; 
    if (pTipo === TipoProduto.CONSUMIVEL || pTipo === 'Consumível' || pTipo === 'Consumivel') return false;
  ...
  const confirmPayment = async () => {
    ...
    const valorPagoNum = isAgendado && valorPago !== '' ? parseFloat(valorPago) : total;
  ...
  const fecharMutation = useMutation({
    mutationFn: ({ id, payload }: { id: number | string; payload: any }) => financialService.fechar(id, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["caixas"] });
  ...
  const movimentoMutation = useMutation({
    mutationFn: ({ tipo, valor }: { tipo: string, valor: number }) => financialService.movimento(String(caixaId), tipo, valor, tipo),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["caixas"] });
  ...
  const handleFecharCaixa = () => {
    ...
    Swal.fire({
      ...
      cancelButtonText: 'Cancelar',
      preConfirm: () => {
        const dinheiro = parseFloat((document.getElementById('swal-dinheiro') as HTMLInputElement).value || '0');
        const transferencia = parseFloat((document.getElementById('swal-transferencia') as HTMLInputElement).value || '0');
        const pos = parseFloat((document.getElementById('swal-pos') as HTMLInputElement).value || '0');
        const justificativa = (document.getElementById('swal-justificativa') as HTMLTextAreaElement).value;
  ...
  const handleSangria = () => {
    ...
    }).then(r => r.isConfirmed && movimentoMutation.mutate({ tipo: 'Sangria', valor: Number(r.value) }));
  };
  ...
  const handleReforco = () => {
    ...
    Swal.fire({
      ...
      showCancelButton: true,
      confirmButtonText: 'Registar',
    }).then(r => r.isConfirmed && movimentoMutation.mutate({ tipo: 'Reforço', valor: Number(r.value) }));
  };
  ...
  if (!isCaixaAberta) {
    return (
      <div className="flex flex-col items-center justify-center h-[calc(100vh-8rem)] bg-surface dark:bg-surface-dark border border-gray-200 dark:border-border-dark rounded-xl shadow-sm animate-fade-in-up">
        ...
        <p className="text-gray-500 mb-8 text-center max-w-md">O caixa encontra-se fechado. Para registar vendas e operações ao balcão, inicie o turno preenchendo o fundo de maneio.</p>
        <button 
    ...
    <div className="h-[calc(100vh-8rem)] flex gap-6 overflow-hidden animate-fade-in">
      <div className="flex-1 bg-surface dark:bg-surface-dark border border-gray-200 dark:border-border-dark rounded-xl flex flex-col overflow-hidden shadow-sm">
        ...
        <div className="bg-white dark:bg-surface-dark border-b border-gray-200 dark:border-border-dark px-4 py-2 shrink-0 flex gap-2 overflow-x-auto">
          {['Tudo', 'Produtos Acabados', 'Revenda', 'Material', 'Menu Eventos', 'Espaço'].map(cat => (
             <button 
               key={cat} 
               onClick={() => setActiveCategory(cat)} 
               className={cn("px-3 py-1.5 text-sm font-medium rounded-full whitespace-nowrap transition-colors border", activeCategory === cat ? 'bg-primary text-white border-primary' : 'bg-gray-100 text-gray-700 border-gray-200 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700')}
             >
                {cat}
             </button>
          ))}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {displayProducts.map((p: any) => (
              <button 
                ...
                className="bg-white dark:bg-surface-dark border border-gray-200 dark:border-gray-800 rounded-xl p-4 text-left hover:border-primary/50 hover:shadow-md transition-all flex flex-col justify-between aspect-square"
              >
                <div>
                  <span className="text-xs font-semibold px-2 py-1 bg-gray-100 dark:bg-gray-800 rounded text-gray-600 dark:text-gray-300">
                ...
                <div className="mt-4">
                  <p className="text-lg font-bold text-primary">{formatCurrency(p.salePrice || p.preco_venda)}</p>
                  <p className="text-xs text-success">{p.quantity || p.stock_atual} {p.unit || p.unidade} em stock</p>
                </div>
              </button>
            ))}
          </div>
      ...
      <div className="w-96 bg-surface dark:bg-surface-dark border border-gray-200 dark:border-border-dark rounded-xl flex flex-col shrink-0 shadow-sm overflow-hidden">
          ...
          <>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              {cart.length === 0 ? (
                ...
                cart.map(item => (
                  <div key={item.id} className="flex flex-col gap-2 p-3 bg-gray-50 dark:bg-gray-800/20 rounded-lg border border-gray-100 dark:border-gray-800">
                    <div className="flex justify-between items-start">
                      <span className="font-medium text-sm text-gray-900 dark:text-white line-clamp-2">{item.name || item.nome}</span>
                      <button onClick={() => removeItem(item.id)} className="text-gray-400 hover:text-error transition-colors">
                        <Trash2 size={16} />
                      </button>
                    </div>
                    <div className="flex items-center justify-between mt-1">
                      <div className="flex items-center gap-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md">
                        <button onClick={() => updateQty(item.id, -1)} className="px-2 py-1 text-gray-500 hover:text-primary"><Minus size={14} /></button>
                        <span className="w-6 text-center text-sm font-semibold">{item.qty}</span>
                        <button onClick={() => updateQty(item.id, 1)} className="px-2 py-1 text-gray-500 hover:text-primary"><Plus size={14} /></button>
                      </div>
                      <span className="font-bold text-sm">{formatCurrency((item.salePrice || item.preco_venda || 0) * item.qty)}</span>
                    </div>
                  </div>
                ))
              )}
        ...
        {step === 2 && (
          <>
            <div className="p-4 border-b border-gray-200 dark:border-border-dark flex items-center justify-between shrink-0">
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Pagamento / Encomenda</h2>
              <button onClick={() => setStep(1)} className="text-sm text-gray-500 hover:text-primary font-bold">Voltar</button>
            </div>
            
            <div className="p-5 flex-1 overflow-y-auto space-y-4">
              {/* Tipo de Pedido: Imediato vs Agendado */}
              <div>
                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-2">Tipo de Atendimento</label>
              ...
              {tipoPedido === 'Agendado' && (
                <div className="p-3 bg-gray-50 dark:bg-gray-850 rounded-xl border border-gray-100 dark:border-gray-800 space-y-3">
                  <h3 className="text-xs font-bold text-primary uppercase">Configurações de Agendamento</h3>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Data e Hora de Entrega *</label>
                    <input
                      type="datetime-local"
                      value={dataEntrega}
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Valor do Sinal / Depósito (STD)</label>
                    <input
                      type="number"
              ...
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">Resumo de Custo</p>
              <div className="flex justify-between items-baseline">
                <span className="text-sm font-semibold text-gray-500">Valor total a faturar:</span>
                <span className="text-2xl font-black text-gray-900 dark:text-white tracking-tight">
                  {formatCurrency(tipoPedido === 'Agendado' && valorPago !== '' ? parseFloat(valorPago) : total)}
                </span>
              </div>
              
              <div className="space-y-2">
                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Método de Cobrança</label>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={() => setPaymentMethod('Dinheiro')} className={cn("py-2.5 rounded-lg border text-xs flex flex-col items-center gap-1 transition-all font-semibold", paymentMethod === 'Dinheiro' ? "bg-primary/10 border-primary text-primary" : "border-gray-200 text-gray-600 dark:border-gray-700 dark:text-gray-300 hover:bg-gray-50")}>
                    <Banknote size={18} /> Dinheiro
                  </button>
                  <button onClick={() => setPaymentMethod('TPA / POS')} className={cn("py-2.5 rounded-lg border text-xs flex flex-col items-center gap-1 transition-all font-semibold", paymentMethod === 'TPA / POS' ? "bg-primary/10 border-primary text-primary" : "border-gray-200 text-gray-600 dark:border-gray-700 dark:text-gray-300 hover:bg-gray-50")}>
                    <CreditCard size={18} /> TPA / POS
                  </button>
                  <button onClick={() => setPaymentMethod('Transferência')} className={cn("py-2.5 rounded-lg border text-xs flex flex-col items-center gap-1 transition-all font-semibold col-span-2", paymentMethod === 'Transferência' ? "bg-primary/10 border-primary text-primary" : "border-gray-200 text-gray-600 dark:border-gray-700 dark:text-gray-300 hover:bg-gray-50")}>
              ...
              {(paymentMethod === 'Transferência' || paymentMethod === 'TPA / POS') && (
                <div className="p-3 bg-amber-50/50 dark:bg-amber-950/20 rounded-xl border border-amber-100 dark:border-amber-900/30 space-y-3">
                  <h4 className="text-xs font-bold text-amber-800 dark:text-amber-400 uppercase">Dados da Transação</h4>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Código de Confirmação / Comprovativo *</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 uppercase mb-1">Emissor / Titular da Conta / Banco *</label>
                    <input
                      type="text"
              ...
              {paymentMethod === 'Dinheiro' && (
                <div>
                  <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">Valor Recebido (STD)</label>
                  <input 
                    type="number" 
                  ...
                  />
                  {amountReceived > (tipoPedido === 'Agendado' && valorPago !== '' ? parseFloat(valorPago) : total) && (
                    <div className="mt-3 p-3 bg-orange-50 dark:bg-orange-950/20 border border-orange-200 dark:border-orange-900/30 rounded-xl flex justify-between items-baseline">
                      <span className="font-bold text-xs text-orange-800 dark:text-orange-400">Troco a devolver:</span>
                      <span className="font-bold text-base text-orange-600 dark:text-orange-300">
                        {formatCurrency(amountReceived - (tipoPedido === 'Agendado' && valorPago !== '' ? parseFloat(valorPago) : total))}
                      </span>
                    </div>
                  )}
                </div>
            ...
            <div className="p-4 border-t border-gray-200 dark:border-border-dark shrink-0">
              ...
              >
                {checkoutPedido.isPending || createVenda.isPending ? 'A PROCESSAR...' : 'CONFIRMAR E REGISTAR'}
              </button>
            </div>
          </>
        )}
        ...
        {step === 3 && (
          <div className="p-6 flex-1 flex flex-col items-center justify-center text-center animate-fade-in-up">
            ...
            <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-1">Sucesso!</h2>
                ...
                {invoiceSent ? (
                  ...
                  <form onSubmit={(e) => {
                    ...
                    enviarFatura.mutate({
                      ...
                      onSuccess: () => {
                        setInvoiceSent(true);
                        toast.success(`Fatura solicitada para envio via ${sendMethod === 'email' ? 'E-mail' : 'WhatsApp'}!`);
                      }
                    });
                  }} className="space-y-3">
                    ...
                    <div className="flex gap-2">
                      <input 
                        ...
                        placeholder={sendMethod === 'email' ? 'exemplo@cliente.com' : 'Telemóvel'}
                        value={sendContact}
